async function search() {
    ...
    try {
        // 2. Xác định đường dẫn đúng đến tệp books.json (tự động)
        const basePath = window.location.pathname.includes('/footer/')
            ? '..'
            : '.';

        const response = await fetch(`${basePath}/books.json`);
        ...
        // 5. Hiển thị kết quả tìm kiếm ra màn hình
